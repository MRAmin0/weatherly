...
class WeatherStore extends ChangeNotifier {
  ...
  Future<void> _init() async {
    ...
    if (_currentLat != null && _currentLon != null) {
      ...
      final resolved = await _apiService.resolveCity(_location);
      if (resolved != null) {
  ...
  Future<void> _fetchWeatherAndForecast({
    ...
    try {
      ...
      if ((targetLat == null || targetLon == null) &&
        ...
        final resolved = await _apiService.resolveCity(trimmedCity);
        if (resolved != null) {
          targetLat = (resolved['lat'] as num?)?.toDouble();
          targetLon = (resolved['lon'] as num?)?.toDouble();
          resolvedLabel = buildCityLabel(resolved);
        }
      }
      ...
      final forecastList = (targetLat != null && targetLon != null)
          ? await _apiService.fetchForecast(lat: targetLat, lon: targetLon)
          : const <dynamic>[];
  ...
  Future<void> _fetchCitySuggestions(String query) async {
    if (!_apiReady) return;
    final results = await _apiService.fetchCitySuggestions(query);
  ...
  void selectCity(Map<String, dynamic> cityData) {
    ...
    _location = buildCityLabel(cityData);
